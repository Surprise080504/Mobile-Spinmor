...
function SandboxSplitPayment({
  ...
  return (
    <React.Fragment>
      {payCardStatus.split(" ")[0] === status.error && (
        <Grid item>
          <Typography color="error">
            An error occurred, you were not charged. (error code:{" "}
            {payCardStatus.split(" ")[1]})
          </Typography>
        </Grid>
      )}
      {processOrderStatus.split(" ")[0] === status.error && (
        <Grid item>
          <Typography color="error">
            You were charged but an occurred creating your order invoice: (
            {processOrderStatus.split(" ")[1]})
          </Typography>
        </Grid>
      )}
